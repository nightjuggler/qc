...
def constructQFT(N, verbose=False):
	...
	# gate, and the conditional phase shift gates. Each of these gate matrices, which act
	# on either one qubit (the Hadamard gate) or two adjacent qubits, is "padded" with the
	# appropriate number of 2x2 identity matrices using the Kronecker product. This padding
	# is done by the above helper function.
	#
	...
	for i in xrange(N):
		...
		# Swap x[i] to the front and apply each conditional phase gate along the way
		for j in xrange(i):
			piDivisor = 1 << (i - j)
		...
		# Apply the Hadamard gate to x[i]
